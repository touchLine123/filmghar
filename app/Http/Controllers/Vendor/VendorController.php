...
class VendorController extends Controller {
    ...
    public function dashboard() {
        ...
        $item['total']   = Item::where('addedBy', $admin->id)->count();
        $item['single']  = Item::where('addedBy', $admin->id)->singleItems()->count();
        $item['episode'] = Item::where('addedBy', $admin->id)->episodeItems()->count();
        $item['trailer'] = Item::where('addedBy', $admin->id)->trailerItems()->count();
        $views           = Item::where('addedBy', $admin->id)->orderBy('id', 'desc')->limit(20)->get(['id', 'title', 'view']);
        
        
        return view('vendor.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'item', 'views'));
    }
    ...
    public function profile() {
        $pageTitle = 'Profile';
        $admin     = auth('vendor')->user();
        return view('vendor.profile', compact('pageTitle', 'admin'));
    }
    ...
    public function profileUpdate(Request $request) {
        ...
        return to_route('vendor.profile')->withNotify($notify);
    }

    public function password() {
        $pageTitle = 'Password Setting';
        $admin     = auth('vendor')->user();
        return view('vendor.password', compact('pageTitle', 'admin'));
    }
    ...
    public function passwordUpdate(Request $request) {
        ...
        return to_route('vendor.password')->withNotify($notify);
    }
    ...
    public function notifications() {
        ...
        return view('vendor.notifications', compact('pageTitle', 'notifications', 'hasUnread', 'hasNotification'));
    }
    ...
    public function requestReport() {
        ...
        if (!$response || !@$response->status || !@$response->message) {
            return to_route('vendor.dashboard')->withErrors('Something went wrong');
        }
        if ($response->status == 'error') {
            return to_route('vendor.dashboard')->withErrors($response->message);
        }
        $reports = $response->message[0];
        return view('vendor.reports', compact('reports', 'pageTitle'));
    }
