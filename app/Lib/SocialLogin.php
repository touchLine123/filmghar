...
class SocialLogin {
    ...
    public function login() {
        ...
        if ($this->fromApi) {
            try {
                $userObject = $driver->stateless()->userFromToken(request()->token);
                $user = (object) $userObject->user;
        
                // Log the retrieved user
                Log::info('Socialite API User Retrieved', [
                    'token' => request()->token,
                    'user' => $user,
                ]);
        
            } catch (\Throwable $th) {
                Log::error('Socialite API Token Failed', [
                    'token' => request()->token,
                    'error' => $th->getMessage(),
            ...
            // Log the user from web flow
            Log::info('Socialite Web User Retrieved', [
                'user' => $user,
            ]);
        }
