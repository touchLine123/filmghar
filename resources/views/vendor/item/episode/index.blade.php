            ...
            <div class="card  ">
                                            ...
                                            <div class="button--group">
                                                ...
                                                @if ($episode->video)
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item threshold" href="{{ route('vendor.item.episode.updateVideo', $episode->id) }}">
                                                            <i class="la la-cloud-upload-alt"></i> @lang('Update Video')
                                                        </a>
                                                        <a class="dropdown-item threshold" href="{{ route('vendor.item.ads.duration', [$episode->item_id, $episode->id]) }}">
                                                            <i class="lab la-buysellads"></i> @lang('Update Ads')
                                                        </a>
                                                        <a class="dropdown-item threshold" href="{{ route('vendor.item.subtitle.list', [$episode->id, $episode->video->id]) }}">
                                                            <i class="las la-file-audio"></i> @lang('Subtitles')
                                                        </a>
                                                        <a class="dropdown-item threshold" href="{{ route('vendor.item.report', [$episode->item_id, $episode->id]) }}">
                                                            <i class="las la-chart-area"></i> @lang('Report')
                                                        </a>
                                                    </div>
                                                @else
                                                    <a class="btn btn--sm btn-outline--warning" href="{{ route('vendor.item.episode.addVideo', $episode->id) }}">
                ...
                @if ($episodes->hasPages())
                    <div class="card-footer py-4">
                        {{ $episodes->links('vendor.partials.paginate') }}
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="modal fade" id="episodeModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('vendor.item.addEpisode', $item->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
    ...
    <script>
        (function($) {
            ...
            $('.addBtn').on('click', function() {
                modal.find('form').attr('action', `{{ route('vendor.item.addEpisode', $item->id) }}`);
            ...
            $('.editBtn').on('click', function() {
                ...
                modal.find('form').attr('action', `{{ route('vendor.item.updateEpisode', '') }}/${data.episode_id}`);
                modal.modal('show');
            });
