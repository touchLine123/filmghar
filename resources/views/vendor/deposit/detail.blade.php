                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Username')
                            <span class="fw-bold">
                                <a href="{{ route('vendor.users.detail', $deposit->user_id) }}"><span>@</span>{{ @$deposit->user->username }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Method')
                ...
                <div class="card overflow-hidden box--shadow1">
                    <div class="card-body">
                        ...
                        @if ($details != null)
                                    ...
                                    <div class="row mt-4">
                                        <div class="col-md-12">
                                            ...
                                            @elseif($val->type == 'file')
                                                @if ($val->value)
                                                    <a href="{{ route('vendor.download.attachment', encrypt(getFilePath('verify') . '/' . $val->value)) }}" class="me-3"><i class="fa-regular fa-file"></i> @lang('Attachment') </a>
                                                @else
                                                    @lang('No File')
                                                @endif
                                            @else
                            ...
                            @endforeach
                            @if ($deposit->method_code < 1000)
                                @include('vendor.deposit.gateway_data', ['details' => json_decode($details)])
                            @endif
                        @endif
                            ...
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <button class="btn btn-outline--success btn--sm ms-1 confirmationBtn"
                                            data-action="{{ route('vendor.deposit.approve', $deposit->id) }}"
    ...
    <div id="rejectModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('vendor.deposit.reject') }}" method="POST">
                    @csrf
